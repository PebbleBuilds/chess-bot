...
def cartesian_to_angles(pos, d1, d2):
    ...
    try:
        ...
        elbow_angle = pi - acos(-1 * (r**2 - d1**2 - d2**2) / (2*d1*d2)) #straight is 0 degrees
        shoulder_angle = pi/2 - (asin(z/r) + atan( (d2*sin(elbow_angle)) / (d1+d2*cos(elbow_angle)))) #straight upwards is 90 degrees
    ...
    shoulder_angle = shoulder_angle / pi * 180
