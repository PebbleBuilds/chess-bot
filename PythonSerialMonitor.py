...
class PythonSerialMonitor():
    def __init__(self):
        ...
        self.cmd_dict = {
            "1": (self.CMD_SET_BASE, 550),
            "2": (self.CMD_SET_SHOULDER, 550),
            "3": (self.CMD_SET_ELBOW, 550)
        }
    ...
    def run(self):
        ...
        while True:
            if not self.input_queue.empty():
                ...
                (cmd_id, cmd_val) = self.cmd_dict.get(keyboardInput,(0,0))

                if cmd_id != 0:
                    try:
                        send_cmd(self.ser, cmd_id, cmd_val)
                    except error:
                        raise error
                else:
